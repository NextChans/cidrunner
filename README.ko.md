...
> [!WARNING]
> **상태: 초기 개발 중.**
> **Phase 4**까지 진행됐습니다. 토폴로지 구성(드래그앤드롭·중첩·규칙 엣지), 실시간
> 검증과 함께 속성 편집, **시작**으로 트래픽(`client → LB → app → DB`) 시뮬레이션,
> 그리고 `terraform validate`를 통과하는 zip **내보내기**까지 됩니다. 미션 완료
> 판정은 아직 없습니다. 로드맵을 참고하세요.
...
| **Phase 4** | Terraform 생성 + zip 내보내기 | ✅ 현재 |
