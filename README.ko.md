...
> [!NOTE]
> **상태: MVP 기능 완성 (Phase 0–5).**
> 토폴로지 구성(드래그앤드롭·중첩·규칙 엣지), 실시간 검증과 함께 속성 편집,
> **시작**으로 트래픽(`client → LB → app → DB`) 시뮬레이션, 별점이 매겨지는 가이드
> **미션** 클리어, 그리고 `terraform validate`를 통과하는 zip **내보내기**까지
> 됩니다. 마감 다듬기와 리소스·미션 추가는 향후 과제입니다.
