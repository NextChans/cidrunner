...
> [!WARNING]
> **상태: 초기 개발 중.**
> **Phase 3**까지 진행됐습니다. 팔레트에서 드래그앤드롭으로 리소스를 배치·중첩하고,
> 규칙이 적용된 엣지로 연결해 토폴로지를 구성하며, 각 리소스의 속성을 실시간 검증과
> 함께 편집하고, **시작**을 눌러 트래픽(`client → LB → app → DB`)을 시뮬레이션하며
> 막힌 지점을 확인할 수 있습니다. Terraform 생성은 아직 없습니다. 로드맵을 참고하세요.
...
| **Phase 3** | 트래픽 시뮬레이션 | ✅ 현재 |
| **Phase 4** | Terraform 생성 + zip 내보내기 | ⬜ 예정 |
| **Phase 5** | 미션 시스템 (클리어 판정 + 별점) | ⬜ 예정 |
