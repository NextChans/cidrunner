...
export const securityHardening: Mission = {
  ...
  hint: 'RDS를 프라이빗 Subnet(퍼블릭 해제)으로 옮기고 스토리지 암호화를 켜세요. SG의 SSH를 끄고, ⚠️ 표시가 남은 리소스에 인스펙터에서 SG를 지정하세요.',
  ...
  check: (ctx) => {
    ...
    let stars = 1
    // No Security Group opens SSH(22) to the world (ADR 0059).
    const noSsh = ctx.securityGroups.every((sg) => !sg.allowSsh)
