...
| 2026-07-09 | 운영 챌린지 티어 (회고 열린 항목 #1 — "예산·카오스를 별점에 반영"): 기존 12미션 게이트는 불변으로 두고, 새 축을 별점으로 소비하는 미션 2종 신설 — **⚡ 무중단 운영**(★2=모든 단일 AZ 장애에서 3-tier 체인 생존: `applyAzFault`+재시뮬+`liveChain` 재판정, 페일오버·복제본 승격 인정; ★3=+오류 0·$90 내)과 **💸 린 서버리스**(★2=월 $5 내+오류 0; ★3=+보안 경고 0). **순차 게이트**(본질 축 미통과 시 상위 별 없음) — 가산식이던 기존 미션과 구별. 두 미션 모두 steps 라이브 체크리스트 탑재(가드 테스트 tutorial-only→3종으로 갱신). 미션 12→**14**. 테스트 198→**204**(ops 6), tsc·oxlint·build clean. 로컬 프리뷰: 이중화 3-tier 공유 URL 로드→무중단 운영 ★3 확인 (ADR 0057). |
| 2026-07-09 | 재생 사운드 (차니 요청 "데이터가 흐를때 소리도"): Web Audio 합성 — 에셋 0. 순수 `soundPlan(sim)`이 `sim.arrivals`(HOP_SECONDS 그리드)에서 이벤트 도출 → 파티클과 자동 동기: **홉 틱**(순번마다 상승 사인파)·**저장 차임**(sink 도달 2음)·**차단 버즈**(sawtooth). fan-out 중복 제거(같은 시각·홉 = 틱 1회)·이벤트 상한 64·**한 패스만**(파티클은 루프해도 소리는 1회). `<SimAudio />` 단일 연결점이 시작 버튼·카오스 재시뮬 모두 커버, 중지/음소거 시 즉시 stop. 음소거 토글(Volume2/VolumeX, 시작 옆) persist 영속 — 교실 사용 대비. AudioContext는 시작 클릭 제스처 안에서 lazy 생성(자동재생 정책 합법). 테스트 204→**208**(soundPlan 4), tsc·oxlint·build clean. Playwright 스모크: 토글·영속·페이지 에러 0 (ADR 0058). |
| 2026-07-10 | 보안 그룹 재설계 (차니 "배선연결이 맞을까?" + 4인 리뷰): SG를 캔버스 노드/엣지에서 제거하고 **스토어 컬렉션 + 리소스 할당**(`config.securityGroupIds`)으로 재모델링 — SG는 흐르는 연결이 아니라 리소스(ENI)에 붙는 다대다 방화벽 규칙이라는 AWS 현실을 반영. 팔레트 하단 **SG 라이브러리**(생성·규칙 토글·삭제) + 인스펙터 **칩 토글 할당**(ENI 보유 리소스만) + 노드 위 **색상 방패 칩** + 같은-SG **hover 하이라이트**. **Terraform 무손실**: export 경계에서 `materializeSecurityGroups`가 합성 sg 노드+엣지로 투영 → `terraform.ts` 및 27개 테스트 불변, tiered ingress·`vpc_security_group_ids` 그대로(실제 `terraform validate` 통과 확인). **영구 마이그레이션**: `sanitizeSnapshot`이 레거시 v1(sg 노드+부착 엣지)을 컬렉션+할당으로 항상 번역(불변 `#g=` URL·localStorage·슬롯 수렴), persist·zundo temporal에 `securityGroups` 편입. 미션 3-tier/시큐리티 하드닝/컨테이너는 config 판정으로 전환. **회귀 함정 해결**: `getGraphIssues` 기본값 `=[]`가 `PropertyForm`의 2-인자 호출에서 매 렌더 새 배열 → 공유 memo 무효화 → React #185 무한 루프. `s.securityGroups` 전달 + 동결 `EMPTY_SGS` 기본값으로 차단. 테스트 214개(신규 securityGroups 6·migration 등), tsc·oxlint·build clean, 브라우저 스모크(할당·칩·에러 0) 확인 (ADR 0059, supersedes 0017·0042). |
| 2026-07-10 | SG 재설계 후속 UX 2건 (차니 보고): (1) **팔레트 레이아웃** — SG 라이브러리가 `flex-1` 리소스 목록 위에 겹쳐 그려지던 것을, 스크롤을 목록 자체로 옮기고 SG 라이브러리를 하단 고정 푸터(자체 max-height 스크롤)로 전환해 해소. (2) **엣지 자동 방향 정렬** (ADR 0060) — ALB처럼 소스+타깃 핸들을 모두 가진 리소스에서 "ALB→EC2"를 왼쪽(타깃) 핸들로 끌면 `EC2→ALB`로 해석돼 거부되던 문제를, `onConnect`에서 그린 방향이 불법이고 반대가 합법이면 조용히 뒤집도록 수정(계층이 단방향 DAG라 안전). 브라우저 재현으로 양방향 확인. 테스트 214, tsc·oxlint·build clean. |
| 2026-07-10 | 관대한 불러오기 (차니 실제 prod topology 업로드 → "불러오기 실패"): `sanitizeSnapshot`이 모르는 리소스 타입(예: ECR·CloudTrail) 하나에 전체 설계를 `return null`로 거부하던 것을, **미지원 노드는 건너뛰고 알리도록** 완화. 구조 손상(비객체·배열 아님·id 비문자열·좌표 비숫자·id 중복)만 하드 실패; 미지원 타입은 스킵+`droppedIds` 기록, 그 노드를 부모로 삼는 자식은 캐스케이드 드롭(부동점), 닿는 엣지·댕글링 엣지도 스킵. `LoadedDesign.unsupportedTypes`로 반환해 "지원하지 않는 리소스는 제외했습니다: …" 토스트 표시. 화이트리스트 재구축(보안)은 유지. 차니 파일(36노드) → 34노드·17엣지·SG 5개 로드 확인(ecr/cloudtrail·엣지 2개만 제외). 테스트 214→**216**(resilient import·cascade), tsc·oxlint·build clean, 브라우저 임포트 스모크(에러 0) (ADR 0061, extends 0020·0023). |
| 2026-07-10 | ECR·CloudTrail 정식 리소스 추가 (차니 topology 후속, AskUserQuestion "둘 다 추가"): 리소스 29→**31**. **ecr**(storage, canvas, `connectsTo: ecs/eks`, 자기완결 `aws_ecr_repository` — 스캔·태그 불변성) + **cloudtrail**(management, canvas, `connectsTo: s3`, `aws_cloudtrail` + `cloudtrail→s3` 엣지에서 해석한 `refs.logBucket` + 파생 `aws_s3_bucket_policy`/`data.aws_caller_identity`로 CloudTrail 쓰기 권한 부여). 두 소스 타입을 트래픽 시뮬에서 제외(이미지 pull·로그 전달은 요청 트래픽 아님). 검증: ECR 컨테이너 미연결·CloudTrail S3 미연결 경고. 차니 prod topology(36노드) → **드롭 0** 완전 로드 + 실제 `terraform validate` 통과(REPLACE_ME 0). 팔레트/임포트 브라우저 스모크(ECR·CloudTrail 노출·제외 토스트 0·에러 0). 테스트 216→**221**(ecr/cloudtrail 5), tsc·oxlint·build clean (ADR 0062). |
| 2026-07-10 | 재생 사운드 루프 (차니 "계속 나면 좋을듯, 생동감 있게"): ADR 0058의 one-pass를 반전. `soundPlan`은 불변, 플레이어만 각 이벤트를 `t0+e.t+k·CYCLE`로 반복 스케줄해 파티클 애니메이션 `CYCLE=1.4s`·노드 `sim-arrival 1.4s infinite` pulse와 **위상 고정**. 룩어헤드 스케줄러(≈2사이클 선예약 + `setInterval(CYCLE)` 보충, Web Audio 클록 재생이라 타이머 지터 무해), 오실레이터 `onended`로 `live` 배열 정리(장시간 누수 방지). `stopSimSounds`가 `loopTimer` clear+stop — 중지·음소거·카오스 재실행 시 즉시 정지. 마스터 게인 0.12 유지. tsc·oxlint·build clean, 오디오 테스트 4 유지 (ADR 0063, reverses 0058). |
| 2026-07-10 | draw.io 내보내기 (차니 요청, AskUserQuestion "Export만"): `graph/drawio.ts` `generateDrawio(nodes, edges, sgs)` — 각 노드를 AWS4 `mxCell`(컨테이너=`mxgraph.aws4.group`+grIcon 그룹, 리프=`resourceIcon`+카테고리색; AZ는 점선 영역)로, mxGeometry는 부모 상대 좌표 그대로·`parent`는 `n-<parentId>`로 **containment 무손실** 매핑(깊이순 정렬), 엣지는 orthogonal `mxCell edge`, SG(노드 아님)는 라벨에 `🛡 이름` 덧붙여 보존. 비압축 `<mxfile>` XML이라 draw.io·diagrams.net·VS Code 확장·Confluence에서 바로 열림, 의존성 0. 툴바 export 버튼(Workflow 아이콘). import은 저충실도라 유보. 차니 topology(36노드) → 36 vertex·19 edge 유효 XML(파서 통과) 확인. 테스트 221→**224**(drawio 3), tsc·oxlint·build clean (ADR 0064). |
| 2026-07-10 | draw.io export 품질 후속 (차니 피드백): (1) **아이콘 토큰 정정** — ecs/ecr/s3/eks/sqs/sns가 긴 서비스명이라 drawio aws4에 없어 빈 블록 → 짧은 토큰으로(검증). (2) **Kinesis 매핑 누락** 추가(fallback 박스→아이콘). (3) 리프 스타일 `strokeColor=#ffffff`. (4) **계정 박스 리플로우** — 부모 없는 글로벌 서비스(WAF/ACM/KMS/…)를 계정 컨테이너 상단 스트립으로 편입하고 기존 자식(VPC)을 아래로 밀며 계정 높이 확장(store 불변, export-layout override map). (5) **모던 그래디언트 템플릿** — AWS 2025/2026 draw.io 라이브러리도 동일한 `mxgraph.aws4.resourceIcon;resIcon=mxgraph.aws4.<name>` 네임스페이스를 쓰므로(글리프는 현행 스텐실로 렌더), 카테고리별 north 그래디언트 필+connection points로 전환해 최신 룩 매칭. 차니 topology 재검증(36 vertex·19 edge·유효 XML). 테스트 225→**227**(kinesis·reflow). tsc·oxlint·build clean (ADR 0064 refinement). |
| 2026-07-10 | draw.io export 간격 수정 (차니 "요소들이 너무 겹쳐있구나"): cidrunner는 넓은 ~175px 카드 기준 좌표인데 draw.io는 78px 아이콘+하단 라벨로 그려 세로로 붙은 행이 겹쳐 보임(3개가 세로 주황블록처럼). 좌표·컨테이너 크기를 `POS_SCALE=1.7`배(리프 아이콘은 78px 고정)로 스케일해 간격 확보(세로 90→153px). 배치·중첩 구조 불변. 차니 topology 재검증. 테스트 227 유지, tsc·oxlint·build clean (ADR 0064 refinement). |
| 2026-07-10 | 강사용 커스텀 미션 (다음 스텝 투표 1위): liveChain이 데이터 주도라는 점을 활용해 미션을 코드 없이 저작·배포. `missions/custom.ts` `CustomMissionSpec{title,goal,hint?,chain:ResourceType[][],requiredResources?,budget?}` + `sanitizeCustomMission`(화이트리스트) + `toMission`(제네릭 ★1 체인·★2 오류0·★3 보안경고0 루브릭) + base64url `#m=` 코덱(`graph/base64url.ts`로 `#g=`와 공유). 스토어 `customMission` 스펙(persist)+`setCustomMission`(챌린지 활성화)+`showCreateMission`. MissionPanel이 활성 커스텀을 빌트인과 함께 라이브 채점(커스텀 배지). lazy `CreateMission` 모달(제목/목표/힌트/예산+체인 빌더, "시작"/"링크 복사"). App이 `#m=` 로드 시 캔버스 불변·미션만 활성. 부수: 공유 `#g=` 로드가 securityGroups 미전달로 SG 유실되던 잠복 버그(ADR 0059 회귀) 수정. 테스트 227→**230**(custom 3), tsc·oxlint·build clean, 브라우저 스모크(모달·시작·배지·에러0) (ADR 0065). |
| 2026-07-15 | Kinesis 다운스트림 케이스별 (차니 "Firehose면 소비자 불요인데 무조건 경고가 뜬다"): Kinesis 다운스트림을 엣지로 추론 — `kinesis → lambda`=소비(event source mapping), `kinesis → s3`=**Firehose 전송**(소비자 불요, 자동 전송). `connectsTo`를 `['lambda','s3']`로 확장, `checks.ts` 경고는 Lambda 소비자·S3 전송 **둘 다 없을 때만** 발생(문구·simulate 막힘 메시지 갱신). Terraform: `kinesis → s3` 엣지 시 `refs.deliveryBucket`으로 `aws_kinesis_firehose_delivery_stream`(extended_s3, kinesis_source_configuration) + 스트림읽기·S3쓰기 Firehose IAM role emit(apply-ready, brace 균형·REPLACE_ME 0 확인). 시뮬은 무변경 자동 해결(kinesis=entry·s3=sink라 `kinesis→s3` 완결 플로우). data-pipeline 미션은 여전히 Lambda hop 요구(회귀 없음). 테스트 230→**231**(kinesis→s3 firehose 1), tsc·oxlint·build clean (ADR 0066, extends 0035). |
| 2026-07-16 | Well-Architected 심사 캡스톤 미션 (후보 중 레버리지 1위 — 표시 전용 등급을 게임에 편입): Well-Architected 등급(ADR 0054, `grade.ts`)이 계산만 되고 별점에 안 쓰이던 유일한 서브시스템 → 등급을 별점 게이트로 소비하는 캡스톤 미션 신설(`missions/wellArchitectedReview.ts`, 미션 14→**15**). `check`가 `wellArchitectedGrade(ctx…)`(순수 함수, 컨텍스트 무확장) 레터를 별점에 1:1 정렬 — `sim.ok` 전제로 **B→★1·A→★2·S→★3**, C/D·미동작 0. 캔버스 등급 배지와 동일 신호라 목표 투명. 운영 티어 관례대로 자가 점검 3스텝(동작+B/A/S). `requiredResources`에 full-stack(alb·ec2·rds·cloudfront·elasticache) 지정해 S는 네 기둥 균형 유도. 기존 14미션 게이트 불변. 테스트 231→**234**(신규 wellArchitectedReview 3: 빈/미동작 0·bestPractice A 배지 1:1·풀스택 S 3★), tsc·oxlint·build clean, tutorial-steps 가드에 신규 미션 편입 (ADR 0067, extends 0054·0057). |
