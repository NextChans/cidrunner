...
## Sprint F3 (오늘의 마지막 스프린트 — Lambda/APIGW 분리 + 시뮬 정확도 + 트래픽 시각화)

4개 스코프를 한 PR로 마무리. 브랜치 `sprint-f3/lambda-apigw-sim-viz`.

- **F3-1 Lambda + API GW 분리 (F1.5/F2 이월 P0.7)** — 콤보를 함수 전용 `lambda`(API GW v2
  블록 제거, label `Lambda`)와 독립 `apigw`(REST API 6블록 `rest_api`·`resource{proxy+}`·
  `method ANY`·`integration AWS_PROXY`·`deployment`·`stage` + invoke permission)로 분리,
  리소스 **26→27**. `apigw → lambda` 엣지로 integration 타깃 해석, 미연결 시 REPLACE_ME.
  CloudFront 오리진을 `lambda`→`apigw`로 재매핑(execute-api 도메인 + stage origin_path).
  **미션 마이그레이션**: HTTP 진입이 본질인 **서버리스 API만** `apigw → lambda → s3`로 갱신,
  비동기·이벤트·데이터 파이프라인은 Lambda 단독 진입 유지(회귀 락인). ADR 0046.
- **F3-2 시뮬 백트래킹 (QA-002 흡수)** — greedy 단일 경로 tracer를 DFS+백트래킹으로 재작성.
  "sink 도달 경로 하나라도 있으면 성공", 되돌리지 않는 visited로 O(V+E)·사이클 종료, 실패 시
  최장 경로로 block 힌트. F2에서 "의도 동작으로 락인"이라 미뤘던 `simulate-edge.test.ts`를
  백트래킹 의미로 **재설계**(dead 브랜치 먼저 그려도 완결 sibling 발견 락인). ADR 0047.
- **F3-3 로드 밸런싱 애니메이션** — `SimResult.fanout`: 도달·비차단 ALB의 **모든** 타깃 엣지를
  라운드로빈 슬롯팅(≥2 타깃), 엣지 시간차 발사 + ALB 노드 바이올렛 `lb-pulse`. 승리 경로가 1개
  여도 부하 분산이 시각적으로 명확. ADR 0048.
- **F3-4 엣지 in/out 시각 효과** — `SimResult.edgeStatus`(ok/blocked): source 확산 링(out)·
  target 수렴 링(in) + 성공 녹색/차단 빨강 통일. SG/복제/파생 엣지 스타일과 구별 유지. ADR 0049.
- **검증** — 테스트 **152→159**, tsc strict·oxlint·vite build clean(최대 청크 199 kB). 로컬
  프리뷰로 4개 스코프 라이브 확인: apigw 진입(`apigw→lambda→s3`), 백트래킹 우회
  (`alb→ec2b→rds`, dead ec2a 우회), ALB fan-out, 엣지 in/out 링. feature branch → PR → merge.

## 오늘 종합 (2026-07-09)

하루에 스프린트 A~F3 + QA 스윕 + QA-001 hotfix 완주.

- **스프린트/PR**: A · B · C · D · E · F1 · F1.5 · F2 · QA 스윕 · QA-001 hotfix · F3.
- **ADR**: 0024~0049 (총 26건 추가, 누적 **49건**).
- **테스트**: 42 → **159** 케이스.
- **리소스**: 14 → **27종**.
- **미션**: 6 → **12종**.
- **품질 게이트**: tsc strict·oxlint·vite build(최대 청크 199 kB, 500 kB 경고 없음) 전 구간 green.
- **QA**: 스윕 8건 발견 → Major 2건(QA-001 hotfix·QA-002 F3) 해소, Minor/Improvement는 백로그.
